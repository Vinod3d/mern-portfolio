...
const Tools = () => {
  ...
  useEffect(() => {
    const getMyApps = async () => {
      const { data } = await axios.get(`${baseUrl}/api/application/getall`, {
  ...
  return (
    <div className="skills w-full flex flex-col items-center gap-8 sm:gap-12 px-[4%] md:px-[10%] my-20">
      ...
      {loading ? (
        ...
        // Display apps once loaded
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-8">
          {apps.map((app) => (
            <Card
              className="card h-fit p-7 flex flex-col justify-center items-center gap-3"
              key={app._id}
            >
              <img
